...
for i=1:length(neuron_synapse_ids)
    disp(i)
    tic
    [row,col] = find(neuron_array(:,1) == neuron_synapse_ids(i));
    [row1,col1] = find(segment_array(:,1) == neuron_synapse_ids(i));
    [row2,col2] = find(segment_synapse_array(:,1) == neuron_synapse_ids(i));
    ...
    toc
    disp(length(segment_array))
    disp(length(segment_synapse_array))
end
...
save("Geometry_Data.mat",'synapse_density_data','-v7.3');
