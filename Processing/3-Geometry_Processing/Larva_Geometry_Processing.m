...
% Flag to determine whether to collect segment and neuron length data
collect_lengths = true;

% Check if length collection is enabled
if collect_lengths

    % Navigate to the directory containing the processed neuron data
    cd ..
    cd ..
    cd ..
    cd ProcessedData/Larva/Cleaned_Data

    % Retrieve list of all .mat files in the directory
    files = dir('*.mat');

    % Initialize arrays for storing length-related propertie
    neuron_lengths = [];
    
    for i=1:4  % Loop over the first four files
        disp(i)

        try  % Display the current file index

            % Initialize storage arrays for various physical quantities
            if collect_lengths
                segment_lengths = [];
            ...
            % Load processed skeleton data
            filename = files(i).name;
            dataset = load(string(filename));
            dataset = dataset.skeleton_cell;
            
            % Iterate neuron by neuron
            for j=1:size(dataset,1)
                % If cell not empty
                if iscell(dataset{j,2})
                    % Same flag as earlier
                    if collect_lengths
                        % At the segment level, collects lengths,
                        ...
                        for k=1:length(path_set)
                            ...
                            % This loop below is for the more complex
                            % calculation of curvature and torsion, not
                            % relevant to the work in the paper
            ...
            % Saves all physical properties at the segment / neuron level
...
% These sections below read data so that everything can be aggregated
% together
...
% Iterates for each neuron
for i=1:length(neuron_synapse_ids)
    disp(i)
    % Finds the lengths, synapses, curvatures, and torsions for that neuron
    ...
    % Sanity check that we have a consistent number of values
    if length(row1) == length(row2)
        % Aggregates segment level data
        segment_dataset = [segment_dataset;horzcat(segment_lengths(row1),segment_synapses(row2),repelem(neuron_synapse_ids(i),length(row1))',segment_curvatures(row3),segment_torsions(row4))];
        % Additional check
        if ~isempty(row)
            % Additional check
            if length(row) == 1
                % Aggregates neuron level data
                neuron_dataset = [neuron_dataset;neuron_lengths(row),neuron_synapses(i),mean(segment_synapses(row2)),neuron_synapse_ids(i),nanmean(segment_curvatures(row3)),nanmean(segment_torsions(row4))];
            end
        end
    end
end

% Saves neuron and segment data together, in one .mat file
